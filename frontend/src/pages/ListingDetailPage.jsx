    ...
    <Container sx={{ marginTop: "32px", marginBottom: "60px" }}>
      ...
      >
        ...
        <Box>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, color: "#222" }}>
              <PoolIcon sx={{ color: "#717171" }} />
              <Typography variant="body1">Private Pool</Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, color: "#222" }}>
          ...
          {currUser &&
          listing &&
          (currUser._id === listing.owner?._id || listing.owner === currUser._id) ? (
            <Box sx={{ display: "flex", gap: "16px", marginTop: "24px" }}>
              <Button
                variant="outlined"
                startIcon={<EditIcon />}
          ...
          >
            ...
            >
              <Typography variant="h5" sx={{ fontWeight: "bold", color: "#222" }}>
                ₹ {totalPrice ? totalPrice.toLocaleString("en-IN") : (listing.price || dummyListing.price).toLocaleString("en-IN")}
                <span style={{ fontSize: "16px", fontWeight: "normal", color: "#717171" }}>
            ...
            <Box sx={{ border: "1px solid #b0b0b0", borderRadius: "8px", marginBottom: "16px", overflow: "hidden" }}>
              <Box sx={{ display: "flex", borderBottom: "1px solid #b0b0b0" }}>
                <Box sx={{ p: 1.5, flex: 1, borderRight: "1px solid #b0b0b0" }}>
                  ...
                  <input
                    ...
                    style={{ border: "none", outline: "none", width: "100%", color: "#222", fontFamily: "inherit" }}
                  />
                </Box>
                <Box sx={{ p: 1.5, flex: 1 }}>
                  <Typography variant="caption" sx={{ fontWeight: "bold", display: "block", mb: 0.5 }}>
      ...
      <Box sx={{ marginTop: "48px" }}>
        <Divider sx={{ mb: 4 }} />
        <Typography variant="h5" sx={{ fontWeight: "600", mb: 3, color: "#222" }}>
          Reviews ({listing.review?.length || 0})
        </Typography>

        {(!listing.review || listing.review.length === 0) && (
          <Typography variant="body1" sx={{ color: "#717171", fontStyle: "italic" }}>
            No reviews yet for this villa. Be the first to write a review!
          </Typography>
        )}
        ...
        <Grid container spacing={3}>
          {listing.review &&
            listing.review.map((rev) => (
              <Grid item xs={12} sm={6} key={rev._id}>
                <Card variant="outlined" sx={{ borderRadius: "12px", boxShadow: "0px 2px 8px rgba(0,0,0,0.05)" }}>
                  <CardContent>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                        <Avatar sx={{ width: 40, height: 40, bgcolor: "#1976d2" }}>
                          {rev.author?.username ? rev.author.username[0].toUpperCase() : "U"}
                        </Avatar>
                        <Box>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {rev.createdAt ? new Date(rev.createdAt).toLocaleDateString("en-IN") : "Recently"}
                          </Typography>
                        </Box>
                      </Box>

                      {currUser && rev.author && (() => {
                        const reviewAuthorId = rev.author._id ? rev.author._id.toString() : rev.author.toString();
                        const loggedInUserId = currUser._id ? currUser._id.toString() : currUser.toString();
                        return reviewAuthorId === loggedInUserId;
                      })() && (
                        <Box sx={{ display: "flex", gap: 0.5 }}>
                          <IconButton color="primary" onClick={() => handleEditReview(rev)} size="small" sx={{ color: "#007A87" }}>
                            <EditIcon fontSize="small" />
                          </IconButton>
                          <IconButton color="error" onClick={() => handleReviewDelete(rev._id)} size="small">
                            <DeleteIcon fontSize="small" />
                          </IconButton>
                        </Box>
                      )}
                    ...
                    <Paper elevation={0} sx={{ backgroundColor: "transparent" }}>
                      <Rating value={rev.rating || 5} readOnly size="small" sx={{ mb: 1 }} />
                    </Paper>

                    <Typography variant="body2" sx={{ color: "#484848", lineHeight: "1.5" }}>
                      {rev.comment}
                    </Typography>
                  </CardContent>
                </Card>
